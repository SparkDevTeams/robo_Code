...
def keydown(e):
    ...
    if e.char == 'a':
        ...
        motors.leftSpeed(currSpeed,0,5,6,11)
        motors.motorOff(1,4,8,9)
        motors.motorOn(2,3,7,10)
    ...
    if e.char == 'd':
        ...
        motors.leftSpeed(currSpeed,0,5,6,11)
        motors.motorOff(2,3,7,10)
        motors.motorOn(1,4,8,9)
...
def displayUltraviolet():
    ultra = ultra_distance.distance()
    repr(ultra)
    sensor = ultra
    sensor3data.config(text = sensor)

def displayGas():
    LPG = gasOutput.LPGLevel()
    CO = gasOutput.COLevel()
    Smoke = gasOutput.SmokeLevel()
    repr(LPG)
    repr(CO)
    repr(Smoke)
    sensor2data.config(text = LPG + "\n" + CO + "\n" + Smoke + "\n")

def displayWater():
    agua = water_level.waterData()
    sensor4data.config(text = agua)
#-------------------------------------------------------------------------------------------------------------------------------------------
...
f2.pack(side = RIGHT)
f2.pack_propagate(0)
lblInfo = Label(Tops, font = ('arial', 70, 'bold'), text = "     BIRD-E GUI     " , bd = 10)
lblInfo.pack()
#------------------------------------------------------------------------------------------------------------------------------------------
...
sensor1data = Label(flbab, font = ('arial', 12, 'bold'), text = "Its miami, you already know.", bd = 4)
