...
class MQ():
    ...
    MQ_PIN                       = 15        # define which analog input channel you are going to use (MCP3008)
    ...
    def __init__(self, Ro=10, analogPin=15):
        self.Ro = Ro
    ...
    def MQResistanceCalculation(self, raw_adc):
        if raw_adc != 0:
            return float(self.RL_VALUE*(1023.0-(raw_adc))/(raw_adc));
    ...
    def MQCalibration(self, mq_pin):
        ...
        for i in range(self.CALIBARAION_SAMPLE_TIMES):          # take multiple samples
            val += self.MQResistanceCalculation(sensor.sensorData(channel))
    ...
    def MQRead(self, mq_pin):
        ...
        for i in range(self.READ_SAMPLE_TIMES):
            rs += self.MQResistanceCalculation(sensor.sensorData(channel))
